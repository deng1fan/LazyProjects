...
def get_tokenized_data(config, tokenizer, model, only_test=False):
    """
    获取tokenized数据集的入口函数
    :param config: dict
    ...
    """
    data_processor = get_data_processor(config, tokenizer, model, only_test)
    return data_processor.get_dataset()


@rank_zero_only
def print_dataset_overview(train_data_tokenized, valid_data_tokenized, test_data_tokenized):
    style = "bold cyan"
    tree = rich.tree.Tree("Dataset Overview", style=style, highlight=True, guide_style=style)
    if train_data_tokenized is not None:
        branch = tree.add("Train Dataset", style=style, guide_style=style)
        branch.add("Size: " + str(len(train_data_tokenized)), style=style, guide_style=style)
        branch.add("Columns: " + '、'.join(train_data_tokenized.column_names), style=style, guide_style=style)

    if valid_data_tokenized is not None:
        branch = tree.add("Valid Dataset", style=style, guide_style=style)
        branch.add("Size: " + str(len(valid_data_tokenized)), style=style, guide_style=style)
        branch.add("Columns: " + '、'.join(valid_data_tokenized.column_names), style=style, guide_style=style)

    if test_data_tokenized is not None:
        branch = tree.add("Test Dataset", style=style, guide_style=style)
        branch.add("Size: " + str(len(test_data_tokenized)), style=style, guide_style=style)
        branch.add("Columns: " + '、'.join(test_data_tokenized.column_names), style=style, guide_style=style)
    rich.print(tree)
...
def read_by(path, data_name=''):
    ...
    if not os.path.exists(path):
        pp(f"文件路径似乎并不存在....")
    pp(f'正在加载文件 {data_name} from {path}')
    if '.pkl' in path:
...
def save_as(data, save_path, file_format="pt", data_name='', protocol=4):
    ...
    if not os.path.exists(parent_path):
        pp(f"保存路径的父文件夹（{parent_path}）不存在，将自动创建....")
        os.makedirs(parent_path)
    save_path = save_path + f'.{file_format}'
    pp(f'正在保存文件 {data_name} 到 {save_path}')
    if file_format == 'pkl':
    ...
    pp(f'保存 {data_name} 成功!')
    return None


#
# def save_list_as_txt(data, save_path, data_name=''):
#     print(f'Save {data_name} to {save_path}')
#     with open(save_path, 'w') as f:
#         for i in data:
#             f.write(i)
#     print(f'Save {data_name} succeed!')
#     pass
...
def read_txt_by_line(file_path, data_name=None):
    ...
    if data_name is not None:
        pp(f'Load {data_name} from {file_path}')
    return all_lines


def split_data(ori_data, random_seed, valid_size=0.1, test_size=0.1):
    train_data, test_data = train_test_split(ori_data, test_size=test_size, random_state=random_seed)
    train_data, valid_data = train_test_split(train_data, test_size=valid_size, random_state=random_seed)
    return train_data, valid_data, test_data


def concatenate_multi_datasets(dataset1, dataset2):
    return Dataset.from_pandas(pd.concat([pd.DataFrame(dataset1), pd.DataFrame(dataset2)], axis=1))
...
def generate_square_subsequent_mask(seq_tensor):
    ...
    mask = (torch.triu(torch.ones((seq_len, seq_len), device=seq_tensor.device)) == 1).transpose(0, 1)
...
@rank_zero_only
def print_sample_data(tokenizer, data_tokenized_list, data_tokenized_name_list, experiment=None, config=None):
    ...
    for i, data_tokenized in enumerate(data_tokenized_list):
        console.print(f"[bold red]{data_tokenized_name_list[i]}数据格式展示", justify='center')
        log_sub_data = {}
        ...
        for c in show_columns:
            if c in data_tokenized.column_names:
                ...
                input_to_print = input_to_print if (
                            config.data_mode == 'classification' and c == 'labels') else tokenizer.decode(
...
def extract_en_keywords_by_keybert(full_text, keyphrase_ngram_range=(1, 3), highlight=False, top_n=10):
...
def extract_en_keywords_by_yake(text, max_ngram_size=2,
                                deduplication_threshold=0.9, top_k=5,
                                language='en'):
    """
...
def extract_en_keywords_by_sklearn_tfidf(corpus, top_n=5):
    ...
    import numpy as np
    # 首先，構建語料庫corpus
    transfer = TfidfVectorizer(stop_words=nltk.corpus.stopwords.words('english'))
...
def flat(input_list):
    ...
    展平列表
    """
    return list(itertools.chain(*[i if isinstance(i, list) else [i] for i in input_list]))
