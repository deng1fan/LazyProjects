...
from general_files.utils.common_util import print_dict_to_table, Result, get_logger, print_error_info
...
def get_bert_score(df, config):
    ...
    scores = score(generated, target, lang="en", verbose=False, rescale_with_baseline=True, device=config.default_device)[-1].numpy()
...
def compute_loss(
        ...
        model,
        tokenizer):
    input_ids, labels, decoder_input_ids = torch.LongTensor(batch['input_ids']).to(model.device), \
                                           torch.LongTensor(batch['labels']).to(model.device), \
                                           torch.LongTensor(batch['decoder_input_ids']).to(model.device)
    ignore_columns = ['source', 'target', 'other_features', 'generated', 'input_ids', 'labels', 'decoder_input',
                      'decoder_prompt', 'decoder_input_ids']
    other_features = dict()
...
def compute_sent_bleu(references, candidates):
    ...
    for example_id, (label, pred) in enumerate(zip(ref_list, dec_list)):
        bleu1 += sentence_bleu(label, pred, weights=[1, 0, 0, 0], smoothing_function=SmoothingFunction().method3)
        bleu2 += sentence_bleu(label, pred, weights=[0.5, 0.5, 0, 0],
                               smoothing_function=SmoothingFunction().method3)
        bleu3 += sentence_bleu(label, pred, weights=[1 / 3, 1 / 3, 1 / 3, 0],
                               smoothing_function=SmoothingFunction().method3)
        bleu4 += sentence_bleu(label, pred, weights=[0.25, 0.25, 0.25, 0.25],
                               smoothing_function=SmoothingFunction().method3)
    bleu1 = bleu1 / len(ref_list)
    bleu2 = bleu2 / len(ref_list)
    bleu3 = bleu3 / len(ref_list)
    bleu4 = bleu4 / len(ref_list)
    return round(bleu1 * 100, 4), round(bleu2 * 100, 4), round(bleu3 * 100, 4), round(bleu4 * 100, 4)
...
def compute_rouge(references, candidates):
    ...
    return round(np.mean(rouge_1), 4), \
...
def get_eval_metrics(test_df, model, config, tokenizer):
    """
    评价指标计算
    :param config:
    ...
    if 'sent_bleu' in eval_metrics:
        ...
        test_result.add(sent_bleu1=bleu1,
                        sent_bleu2=bleu2,
                        sent_bleu3=bleu3,
                        sent_bleu4=bleu4, )
    ...
    if 'corpus_bleu' in eval_metrics:
        ...
        test_result.add(corpus_bleu1=bleu1,
                        corpus_bleu2=bleu2,
                        corpus_bleu3=bleu3,
                        corpus_bleu4=bleu4, )
    ...
    if 'meteor' in eval_metrics:
        ...
        meteor = compute_meteor(reference, generated_seqs)
        test_result.add(meteor=meteor)
        log.info(f"meteor = {str(meteor)}")
    ...
    if 'bert_score' in eval_metrics:
        ...
        try:
            ...
            log.info(f"bert_score = {str(scores)}")
        except ValueError:
            log.info("Rouge 无法计算，Reference可能为空，请检查生成数据！")
    ...
    print_dict_to_table(test_result, 'Metric', 'Score:yum:',
                        f'[bold green]Test results of {config.model_processor} on {config.dataset_processor}', config)
    return test_result


def pack_result_to_seq2seq_lmoutput(result):
    default_result = Result(loss=None,
                            logits=None,
...
def nucleus_generate(input_ids: torch.Tensor,
                     decoder_input_ids: torch.Tensor,
                     decoder_eos_token_id: int,
                     max_length: int = 100, temperature: float = 1., top_k: int = 0, top_p: float = 0.7,
                     model=None,
                     **other_features):
    generated_ids = None
    past_result = None
    for i in range(max_length):
        ...
        if (pred_index == decoder_eos_token_id).int().sum() == len(pred_index):
            break
        generated_ids = torch.cat([generated_ids, pred_index], dim=-1) if generated_ids is not None else pred_index
        if decoder_input_ids is not None:
            decoder_input_ids = torch.cat([decoder_input_ids, pred_index.to(decoder_input_ids.device)], dim=-1)
        else:
...
def greedy_generate(input_ids: torch.Tensor,
                    decoder_input_ids: torch.Tensor,
                    decoder_eos_token_id: int,
                    max_length: int = 100,
                    model=None,
...
def top_k_top_p_filtering(batch_logits, top_k=10, top_p=0.9, filter_value=-10000.) -> torch.tensor:
...
def generate_sentences(model, batch, tokenizer, config):
    ...
    for key in other_features.keys():
        if isinstance(other_features[key], torch.Tensor):
    ...
    if not generated_ids:
        if config.data_mode == 'unilm':
            generated_ids = [g[len(input_ids[i]):] for i, g in enumerate(ori_generated_ids)]
        else:
    ...
    generated_sentences = [
        {'seqs': tokenizer.decode(sent, skip_special_tokens=True),
         'seqs_with_special_tokens': tokenizer.decode(sent, skip_special_tokens=False, ignore_tokens=[tokenizer.pad_token])} for i, sent in enumerate(generated_ids)]
