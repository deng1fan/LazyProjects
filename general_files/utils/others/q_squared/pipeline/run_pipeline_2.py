...
def get_answer(question, text, qa_model, qa_tokenizer):  # Code taken from https://huggingface.co/transformers/task_summary.html
    inputs = qa_tokenizer.encode_plus(question, text, add_special_tokens=True, return_tensors="pt")#.to("cuda:0")
...
def get_questions_beam(answer, context, qg_model, qg_tokenizer, max_length=128, beam_size=5, num_return=5):
    ...
    features = qg_tokenizer([input_text], return_tensors='pt')#.to("cuda:0")
...
def single_question_score(question, cand, response, knowledge, qa_model, qa_tokenizer):
    pred_ans = get_answer(question, response, qa_model, qa_tokenizer)

    if filter_questions(cand, pred_ans) == 'VALID':
        knowledge_ans = get_answer(question, knowledge, qa_model, qa_tokenizer)
...
def get_response_score(response, knowledge, gen_method, single, remove_personal, qg_model, qg_tokenizer, qa_model, qa_tokenizer):
    ...
    for cand in candidates:
        if gen_method == 'greedy':
            ...
            questions = get_questions_beam(cand, response, qg_model, qg_tokenizer)
        else:
...
def calc_scores(response, knowledge, gen_method='beam', single=True, remove_personal=True):
    ...
    # torch.save(data, '/home/dengyf/code/exp_1/general_files/utils/q_squared/third_party/data.pt')
    data = torch.load('/home/dengyf/code/exp_1/general_files/utils/q_squared/third_party/data.pt')
...
if __name__ == '__main__':
    ...
    ds = Dataset.from_csv("/home/dengyf/code/exp_1/general_files/utils/q_squared/third_party/data/dodeca_consistent.csv")
...
#     parser.add_argument("--infile", type=str, default="/home/dengyf/code/exp_1/general_files/utils/q_squared/third_party/data/dodeca_inconsistent.csv",
