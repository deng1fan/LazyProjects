...
def train(config: DictConfig, experiment):
    ...
    if os.path.exists(config.cache_dataset_path + '_preprocess_dataset.pt') and not config.force_reload_data and not config.fast_run:
        ...
        if not config.fast_run:
            log.info(f"保存数据集缓存...: {config.cache_dataset_path}_preprocess_dataset.pt")
    ...
    save_as(test_output, config.result_path + '/test_output', data_name="测试输出")
    ...
    if config.data_mode != 'classification':
        # 保存测试生成语句方便以后测试
        print_generated_dialogs(test_output, experiment, mode=config.data_mode, config=config)
    log.info("评估模型！")
